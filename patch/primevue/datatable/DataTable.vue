  ...
  <div :class="containerClass">
    ...
    <div class="p-datatable-scrollable-wrapper" v-else>
      ...
      >
        <template #header>
          <DTTableHeader
              :columnGroup="frozenHeaderColumnGroup"
              :columns="frozenColumns"
              :rowGroupMode="rowGroupMode"
              :groupRowsBy="groupRowsBy"
              :resizableColumns="resizableColumns"
...
export default {
  ...
  methods: {
    ...
    onColumnHeaderClick (e) {
      ...
      if (column.sortable) {
        ...
        if (DomHandler.hasClass(targetNode, 'p-sortable-column') || DomHandler.hasClass(targetNode,
            'p-column-title',
        ) || DomHandler.hasClass(
            targetNode,
    ...
    sortSingle (value) {
      ...
      data.sort((data1, data2) => {
        ...
        if (value1 == null && value2 != null) result = -1; else if (value1 != null && value2 == null) result = 1; else if (value1 == null && value2 == null) result = 0; else if (typeof value1 === 'string' && typeof value2 === 'string') result = value1.localeCompare(value2,
            undefined,
            { numeric: true },
        ); else result = (
                             value1 < value2
    ...
    multisortField (data1, data2, index) {
      ...
      if (typeof value1 === 'string' || value1 instanceof String) {
        if (value1.localeCompare && (
          ...
          return (
              this.d_multiSortMeta[index].order * value1.localeCompare(value2, undefined, { numeric: true })
          );
        }
      } else {
        result = (
      ...
      return (
          this.d_multiSortMeta[index].order * result
      );
    },
    addMultiSortField (field) {
      ...
      if (index >= 0) {
        ...
        )) this.d_multiSortMeta.splice(index, 1); else this.d_multiSortMeta[index] = {
          field: field,
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        for (let j = 0; j < this.columns.length; j++) {
          ...
          if (!col.excludeGlobalFilter && this.hasGlobalFilter() && !globalMatch) {
            globalMatch = FilterUtils.contains(ObjectUtils.resolveFieldData(data[i], columnField),
                this.filters['global'],
                this.filterLocale,
            );
          }
    ...
    findPrevSelectableRow (row) {
      ...
      if (prevRow) {
        if (DomHandler.hasClass(prevRow, 'p-selectable-row')) return prevRow; else return this.findPrevSelectableRow(
            prevRow);
      } else {
        return null;
      }
    ...
    toggleRowsWithCheckbox (event) {
      ...
      const _selection = checked ? [] : (
          processedData ? [...processedData] : [...this.value]
      );
      this.$emit('update:selection', _selection);

      if (checked) this.$emit('row-unselect-all', { originalEvent: event }); else this.$emit('row-select-all', {
        originalEvent: event,
        data: _selection,
      });
    },
    ...
    isSelected (rowData) {
      if (rowData && this.selection) {
        if (this.dataKey) {
          return this.d_selectionKeys ? this.d_selectionKeys[ObjectUtils.resolveFieldData(rowData,
              this.dataKey,
          )] !== undefined : false;
        } else {
          if (this.selection instanceof Array) return this.findIndexInSelection(rowData) > -1; else return this.equals(rowData,
              this.selection,
          );
        }
      }
        ...
        if (this.columnResizeMode === 'fit') {
          ...
          if (newColumnWidth > 15 && nextColumnWidth > 15) {
            if (this.scrollable) {
              ...
              const scrollableBodyTable = DomHandler.findSingle(scrollableView,
                  'table.p-datatable-scrollable-body-table',
              );
              const scrollableHeaderTable = DomHandler.findSingle(scrollableView,
                  'table.p-datatable-scrollable-header-table',
              );
              const scrollableFooterTable = DomHandler.findSingle(scrollableView,
    ...
    onColumnHeaderMouseDown (e) {
      ...
      if (this.reorderableColumns && column.reorderableColumn) {
        if (event.target.nodeName === 'INPUT' || event.target.nodeName === 'TEXTAREA' || DomHandler.hasClass(event.target,
            'p-column-resizer',
        )) event.currentTarget.draggable = false; else event.currentTarget.draggable = true;
      }
    },
    ...
    onColumnHeaderDragOver (event) {
      ...
      if (this.reorderableColumns && this.draggedColumn && dropHeader) {
        ...
        if (this.draggedColumn !== dropHeader) {
          ...
          this.$refs.reorderIndicatorUp.style.top = dropHeaderOffset.top - containerOffset.top - (
          ...
          if (event.pageX > columnCenter) {
            this.$refs.reorderIndicatorUp.style.left = (
                targetLeft + dropHeader.offsetWidth - Math.ceil(this.colReorderIconWidth / 2)
            ) + 'px';
            ...
            this.$refs.reorderIndicatorUp.style.left = (
                targetLeft - Math.ceil(this.colReorderIconWidth / 2)
            ) + 'px';
            this.$refs.reorderIndicatorDown.style.left = (
                targetLeft - Math.ceil(this.colReorderIconWidth / 2)
            ) + 'px';
    ...
    resizeScrollableTable (column, newColumnWidth, delta) {
      ...
      const scrollableHeaderTable = DomHandler.findSingle(scrollableHeader,
          'table.p-datatable-scrollable-header-table',
      );
      const scrollableFooterTable = DomHandler.findSingle(scrollableFooter,
          'table.p-datatable-scrollable-footer-table',
      );
    ...
    onRowMouseDown (event) {
      if (DomHandler.hasClass(event.target,
          'p-datatable-reorderablerow-handle',
      )) event.currentTarget.draggable = true; else event.currentTarget.draggable = false;
    },
    onRowDragStart (e) {
      const event = e.originalEvent;
      const index = e.index;
    ...
    onRowDragOver (e) {
      ...
      if (this.rowDragging && this.draggedRowIndex !== index) {
        ...
        if (pageY < rowMidY) {
          ...
          if (prevRowElement) DomHandler.addClass(prevRowElement,
              'p-datatable-dragpoint-bottom',
          ); else DomHandler.addClass(
              rowElement,
              'p-datatable-dragpoint-top',
          );
        } else {
          if (prevRowElement) DomHandler.removeClass(prevRowElement,
              'p-datatable-dragpoint-bottom',
          ); else DomHandler.addClass(
              rowElement,
              'p-datatable-dragpoint-top',
    ...
    toggleRow (event) {
      ...
      if (this.dataKey) {
        expanded = this.d_expandedRowKeys ? this.d_expandedRowKeys[ObjectUtils.resolveFieldData(rowData,
            this.dataKey,
        )] !== undefined : false;
      } else {
    ...
    saveColumnWidths (state) {
      ...
      if (this.columnResizeMode === 'expand') {
        state.tableWidth = this.scrollable ? DomHandler.findSingle(this.$el,
            '.p-datatable-scrollable-header-table',
        ).style.width : DomHandler.getOuterWidth(this.$refs.table) + 'px';
      }
    },
    restoreColumnWidths () {
  ...
  computed: {
    ...
    columns () {
      ...
      if (this.allChildren) {
        ...
        if (this.reorderableColumns && this.d_columnOrder) {
          ...
          return [
            ...orderedColumns, ...columns.filter((item) => {
    ...
    processedData () {
      if (this.lazy) {
        ...
        if (this.value && this.value.length) {
          ...
          if (this.sorted) {
            if (this.sortMode === 'single') data = this.sortSingle(data); else if (this.sortMode === 'multiple') data = this.sortMultiple(
                data);
          }
    ...
    allRowsSelected () {
      ...
      return (
          val && val.length > 0 && this.selection && this.selection.length > 0 && this.selection.length === val.length
      );
    },
  },
  components: {
