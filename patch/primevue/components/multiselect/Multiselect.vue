...
export default {
  ...
  methods: {
    ...
    onOptionSelect (event, option) {
      ...
      if (selected) value = this.value.filter(val => !ObjectUtils.equals(
        val,
    ...
    isOutsideClicked (event) {
      return !(
        this.$refs.container.isSameNode(event.target) || this.$refs.container.contains(event.target) || (
    ...
    onToggleAll (event) {
      const value = this.allSelected ? [] : this.visibleOptions && this.visibleOptions.map(option => this.getOptionValue(
        option))
    ...
    labelClass () {
      return [
        'p-multiselect-label', {
          ...
          'p-multiselect-label-empty': !this.$scopedSlots['value'] && !this.placeholder && (
