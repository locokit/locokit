  ...
  methods: {
    ...
    getCurrencyExpression () {
      if (this.currency) {
        ...
        return new RegExp(`[${formatter.format(1).replace(/\s/g, '').replace(this._numeral, '').replace(
          this._decimal,
          ''
    ...
    parseValue (text) {
      let filteredText = text.replace(this._suffix, '').replace(this._prefix, '').trim().replace(
        /\s/g,
        ''
      ).replace(this._currency, '').replace(this._group, '').replace(this._minusSign, '-').replace(
        this._decimal,
        '.'
    ...
    isValueChanged (currentValue, newValue) {
      ...
      if (newValue != null) {
        let parsedCurrentValue = (
                                 ...
                                 ) ? this.parseValue(currentValue) : currentValue
