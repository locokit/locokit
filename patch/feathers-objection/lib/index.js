...
const _errorHandler = _interopRequireDefault(require('./error-handler'))
    ...
    if (Array.isArray(this.id)) {
      ...
      this.id.forEach((idKey, index) => {
        if (!ids) {
          if (idList) {
            if (idList[index]) {
              query[idKey] = idList[index].length === 1
                ? idList[index]
                : {
                    $in: idList[index],
                  }
      ...
      query[addTableName ? `${this.Model.tableName}.${this.id}` : this.id] = idList
        ? idList.length === 1
          ? idList[0]
          : {
              $in: idList,
