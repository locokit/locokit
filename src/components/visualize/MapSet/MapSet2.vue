      ...
      if (columnId) {
        ...
        geoResources.forEach(geoResource => {
          this.$set(this.selectedFeatureBySource, geoResource.id, {
            featureId: `${reference}:${columnId}`,
            center: this.settings.caughtEvents?.[triggerBlockId][catchEventIndex].centerToFeature,
            zoomLevel: this.settings.caughtEvents?.[triggerBlockId][catchEventIndex].zoomToLevel,
          })
        })
      }
    },
