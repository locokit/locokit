...
export default {
  ...
  methods: {
    ...
    onPageReorderClick (chapterId, data) {
      if (chapterId && data && data.moved) {
        ...
        updatedPages[currentPage.id] = { oldPos: currentPage.position, newPos: data.moved.newIndex }
        currentPage.position = updatedPages[currentPage.id].newPos
        ...
        if (data.moved.oldIndex > data.moved.newIndex) {
          ...
          for (let index = data.moved.newIndex; index < data.moved.oldIndex; index++) {
            ...
            updatedPages[currentPage.id] = { oldPos: currentPage.position, newPos: index + 1 }
            currentPage.position = updatedPages[currentPage.id].newPos
          }
        } else {
        ...
        Promise.allSettled(
          ...
          .then(async results => {
            const badResult = results.find(res => res.reason)
            if (badResult) {
              this.displayToastOnError(badResult.reason)
    ...
    displayToastOnError (message, httpCode = null) {
      this.$toast.add({
        severity: 'error',
        summary: httpCode ? this.$t('error.http.' + httpCode) : this.$t('error.basic'),
        detail: message,
        life: 3000
      })
    }
