...
describe('Record abilities', () => {
  ...
  describe('when user is a SUPERADMIN', () => {
    ...
    it('can manage all records', () => {
      expect.assertions(3)
      expect(ability.can('manage', 'record')).toBe(true)
      expect(ability.can('manage', record1)).toBe(true)
      expect(ability.can('manage', record2)).toBe(true)
    })
  })
  ...
  describe('when user is an ADMIN', () => {
    ...
    it('can manage all records - but not all resources', () => {
      expect.assertions(3)
      expect(ability.can('manage', 'record')).toBe(true)
      expect(ability.can('manage', record1)).toBe(true)
      expect(ability.can('manage', record2)).toBe(true)
