...
import { WORKSPACE } from './3-visualize/chapters'
import { DATABASE, TABLES } from '../seeds/templates/v-logistique/glossary/schema'
import { VALUES } from '../seeds/templates/v-logistique/glossary/value_single_select'
...
async function seed1 (knex: Knex): Promise<any> {
  /**
   * Database
   */
  ...
  /**
   * Table Pré-Bénéficiaire
   */
  await knex('table').insert([
    {
      id: TABLES.VLO.LIST_PRE_RECIPIENT.ID,
      text: 'Pré-Bénéficiaire',
      database_id: DATABASE
    }
  ])
  await knex('table_column').insert([
  ...
  await knex('table_column').insert([
    {
      ...
      id: TABLES.VLO.MORIO.COLUMNS.REF,
      text: 'Numéro traceur',
      table_id: TABLES.VLO.MORIO.ID,
      column_type_id: glossary.COLUMN_TYPE.STRING
    }, {
