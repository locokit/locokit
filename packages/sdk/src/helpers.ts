...
import { WorkBook, WorkSheet, XLSX$Utils } from 'xlsx'
...
export async function searchItems({
  ...
  let items: Array<{ label: string; value: string | number }> = []
  if (
    columnTypeId === COLUMN_TYPE.USER ||
    columnTypeId === COLUMN_TYPE.MULTI_USER
  ) {
 ...
 */
export async function searchPageWithChapter(
  query: string,
  filters: object = {},
) {
 ...
 */
export async function searchColumnsFromTableView(
  query: string,
  tableViewId: string,
  filters: object = {},
) {
 ...
 */
export async function searchBooleanColumnsFromTableView(
  query: string,
  tableViewId: string,
) {
  return await searchColumnsFromTableView(query, tableViewId, {
    $or: [
      {
 ...
 */
async function retrieveTableViewData(
  tableViewId: string,
  filters: object = {},
  groupId?: string,
): Promise<{
...
function getValueExport(
  ...
  switch (currentColumn.column_type_id) {
    ...
    default:
      return getColumnDisplayValue(currentColumn, currentRowValue, true) as
        | string
        | undefined
  }
}
 ...
 * Export data in xls
 */
export async function exportTableRowDataXLS(
  tableViewId: string,
  filters: object = {},
  fileName = 'Export',
  groupId?: string,
) {
  const { viewData, viewColumns } = await retrieveTableViewData(
    tableViewId,
    filters,
    groupId,
  )
 ...
 * Export data in csv
 */
export async function exportTableRowDataCSV(
  tableViewId: string,
  filters: object = {},
  fileName: string,
  groupId?: string,
...
export async function uploadMultipleFiles(
  ...
  const uploadPromises: Array<Promise<LckAttachment>> = []
 ...
 */
export async function retrieveWorkspaceWithChaptersAndPages(
  workspaceId: string,
) {
 ...
 */
export async function retrieveWorkspaceUserGroupsWithChaptersAndPages(
  workspaceId: string,
  userId: string,
) {
    ...
    (
      ...
      accumulator.push({
        ...
        chapter: {
          ...
          pages: currentGroup.aclset?.chapter?.pages?.sort(
 ...
 */
export async function retrieveWorkspaceWithDatabases(
  workspaceId: string,
): Promise<LckWorkspace> {
  const workspace: LckWorkspace = await lckServices.workspace.get(workspaceId, {
 ...
 * @param dateFields
 */
function convertDateInRecord(
  record: LckTableRow,
  dateFields: LckTableColumn[],
) {
  dateFields.forEach(({ id, column_type_id }) => {
    ...
    if (column_type_id === COLUMN_TYPE.LOOKED_UP_COLUMN) {
      ...
      ;(record.data[id] as LckTableRowDataComplex).value = parseISO(
        (record.data[id] as LckTableRowDataComplex).value as string,
      )
    } else {
 ...
 */
export function convertDateInRecords(
  records: LckTableRow | LckTableRow[],
  fieldDefinition: LckTableColumn[],
): void {
  const dateFields = fieldDefinition.filter((f) => {
    ...
    switch (f.column_type_id) {
      ...
      case COLUMN_TYPE.FORMULA:
        keepField = [COLUMN_TYPE.DATE, COLUMN_TYPE.DATETIME].includes(
          getColumnTypeId(f),
        )
    }
    return keepField
  ...
  if (Array.isArray(records)) {
    records.forEach((currentRecord) =>
      convertDateInRecord(currentRecord, dateFields),
    )
  } else {
...
export function formatRowData(
  ...
  columnsObject: Record<string, LckTableViewColumn>,
  duplication = false,
) {
  ...
  for (const [columnId, value] of Object.entries(data)) {
    ...
    switch (matchingColumn.column_type_id) {
      case COLUMN_TYPE.DATE:
         ...
         */
        formattedData[columnId] =
          value && isValid(value) ? formatDateISO(value as Date) : null
        break
      case COLUMN_TYPE.DATETIME:
        /**
         ...
         */
        formattedData[columnId] =
          value && isValid(value) ? formatDateTimeISO(value as Date) : null
        break
      case COLUMN_TYPE.FILE:
        /**
      ...
      case COLUMN_TYPE.MULTI_GROUP:
        formattedData[columnId] =
          value && Object.hasOwnProperty.call(value, 'reference')
            ? (value as LckTableRowDataComplex | LCKTableRowMultiDataComplex)
                .reference
      ...
      default:
        if (
          !duplication ||
          (matchingColumn.column_type_id === COLUMN_TYPE.FORMULA &&
            matchingColumn.reference) ||
