...
const _adapterCommons = require('@feathersjs/adapter-commons')

const _errors = _interopRequireDefault(require('@feathersjs/errors'))

const _objection = require('objection')

const _utils = _interopRequireDefault(require('./utils'))

const _errorHandler = _interopRequireDefault(
  require('./src/feathers-objection/error-handler'),
)
...
class Service extends _adapterCommons.AdapterService {
  constructor(options) {
    ...
    this.allowedInsert =
      ...
      _objection.RelationExpression.create(options.allowedInsert)
  ...
  _selectQuery(q, $select) {
    if ($select && Array.isArray($select)) {
      ...
      for (const [key, item] of Object.entries(items)) {
        ...
        if (matches) {
          items[key] = (0, _objection.ref)(matches[1]).as(
            matches[3] || matches[1],
          )
        }
      }
  ...
  _selectAliases($select) {
    ...
    return $select.reduce((result, item) => {
      ...
      if (matches) {
        ...
        // Can't parse $select !
        throw new _errors.default.BadRequest(
          `${item} is not a valid select statement`,
        )
      }
  ...
  _batchInsert(data, params) {
    const { dialect } = this.Model.knex().client // batch insert only works with Postgresql and SQL Server
  ...
  _patch(id, data, params = {}) {
    ...
    const mapIds = (page) =>
      Array.isArray(this.id)
        ...
        : (page.data || page).map((current) => current[this.id]) // By default we will just query for the one id. For multi patch
            ...
            : this._find(findParams).then((page) => {
                ...
                if (id !== null) {
                  if (items.length === 1) {
                    ...
                    throw new _errors.default.NotFound(
