...
const sleep = (s) => new Promise((r) => setTimeout(r, s * 1e3 | 0));
  ...
  return {
    ...
    configResolved(config) {
      ...
      if (config.server && config.define?.["import.meta.env.VITE_FV_DEV_USER"] === void 0) {
        ...
        config.define ||= {};
        const socketUrl = config.mode === "production" ? "globalThis.location" : `globalThis.location.origin.replace('--${VP}', '--${sm.port}').replace(':${VP}', ':${sm.port}')`;
    ...
    async handleHotUpdate({ file, server }) {
      try {
        if (!file.startsWith(sm.root) || sm.active === false || sm.appListener === void 0) {
          return;
        }
        sm.appListener.close();
